...
class core_renderer extends \theme_boost\output\core_renderer {
    ...
    public function body_attributes($additionalclasses = []) {
        ...
        if (isloggedin() && !isguestuser()) {
            $verified = $DB->record_exists('cohort_members', ['cohortid' => 2, 'userid' => $USER->id]);
            if (!$verified) {
                $verified = $DB->record_exists_sql(
                    "SELECT 1
                      ...
                      WHERE d.userid = ?
                        AND f.shortname = ?
                        AND d.data = ?",
                    [$USER->id, 'verification', 'Aprobado']
                );
            }
            if ($verified) {
                $additionalclasses[] = 'impefe-access-approved';
            }
        }
